...
# 키: 날짜+매체+광고목적+캠페인명+광고그룹명+소재명 (전환·인지 계정에 같은 소재명이 있어도 분리 유지)
# ※ 캠페인명이 키에 없으면, 광고그룹명·소재명이 같고 캠페인만 다른 행이 하나로 합쳐져 실적이 사라진다.
#   (유튜브는 '관심사타깃' 같은 광고그룹명을 여러 캠페인에서 재사용해 실제로 손실이 발생했다)
# keep="last": 파일명 정렬상 뒤에 오는(더 최근 수집된) 파일의 값을 우선
before = len(new_df)
new_df = new_df.drop_duplicates(subset=["날짜", "매체", "광고목적", "캠페인명", "광고그룹명", "소재명"], keep="last")
